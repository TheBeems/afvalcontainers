...
const wh24 = JSON.parse(readFileSync(resolve(here, 'wh24-public-column.json'), 'utf8'));
const existing = JSON.parse(readFileSync(resolve(prior, 'existing-11-household-coverage.json'), 'utf8'));
if (wh24.houseIds.join(',') !== matrix.houseIds.join(',')) throw new Error('WH24 column house order differs.');
matrix.candidateIds.push('WH24');
matrix.distances.forEach((row, index) => row.push(wh24.distances[index]));

const fixed = ['WH03', 'WH05', 'WH06', 'WH08', 'WH14', 'WH24', 'WH26', 'WH27', 'WH33', 'WH34'];
const additions = [
...
const finalists = ['M154', 'M157', 'M027', 'WH02', 'M004'];
const privateIds = new Set(existing.houses.filter(({ nearestLocationId }) => nearestLocationId === 'WH23').map(({ id }) => id));
...
function auction(siteIds) {
  ...
  const columns = siteIds.map((id) => candidateIndex.get(id));
...
const results = finalists.map((removed) => ({
  removed,
  ...auction([...fixed, ...additions.filter((id) => id !== removed)])
})).sort((a, b) => a.total - b.total || a.p95 - b.p95 || a.removed.localeCompare(b.removed));

console.log(JSON.stringify({ publicHouseholds: publicIndexes.length, results }, null, 2));
