...
const cards = aerial.sites.map((site) => {
  ...
  return `<figure>
    <div class="photo"><img src="${escapeHtml(site.aerialImage)}" alt="PDOK-orthofoto rond ${escapeHtml(site.id)}, ${escapeHtml(site.referenceAddress)}"><span class="cross horizontal"></span><span class="cross vertical"></span></div>
    <figcaption><strong>${escapeHtml(site.id)}</strong> · ${escapeHtml(site.referenceAddress)} · ${escapeHtml(screen?.rating ?? '')}<br><span>${site.latitude.toFixed(6)}, ${site.longitude.toFixed(6)}</span><br><a href="${escapeHtml(site.aerialSourceUrl)}">Open exact WMS-bronverzoek</a></figcaption>
  </figure>`;
}).join('\n');
